...
class Word2QDM(nn.Module):
    ...
    def __init__(self, vocab_size, layers, num_qubits, num_dm_qubits):
        ...
        self.target_params = nn.Parameter(torch.empty((vocab_size, layers, num_qubits)).normal_(mean=0,std=np.pi))
        self.context_params = nn.Parameter(torch.empty((vocab_size, layers, num_qubits)).normal_(mean=0,std=np.pi))

        self.device = qml.device("default.qubit", wires=num_qubits)

        self.circuit = qml.QNode(self.qmodel, self.device, interface="torch", diff_method="backprop")
    ...
    def forward(self, target_indices, context_indices, neg_indices):
        ...
        for i, t in enumerate(target_indices):
            A_target = self.circuit(self.target_params[t]).reshape(4, 4)
            A_context = self.circuit(self.context_params[context_indices[i]]).reshape(4, 4)
            # TODO: enable multiple neg_samples
            A_neg = self.circuit(self.context_params[neg_indices[i][0]]).reshape(4, 4)
